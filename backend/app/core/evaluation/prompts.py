...
def generate_problem_analysis_prompt(solution_data: dict):
    ...
    """
    # Extract information from problem.json (passed as part of solution_data or separate?)
    # In MMBench run_evaluation.py, solution_data seems to contain 'background' etc.
    # We need to ensure our solution_for_eval.json has these fields.
    ...
    task_analyses = []
    task_number = 1
    # Filter for task keys (assuming they start with "ques" or similar, or just iterate dict excluding metadata)
    # MMBench solution keys are "Question 1", "Question 2".
    # We will iterate and check for dict type values.
